...
* Back – Reverts the model to the previous state, if there are any; For example, if I was using my model from chapter 101 and selected <PLACEHOLDER> and then <PLACEHOLDER>, I could use the Back method to remove the last selections (<placeholder>) that was added. I could use it again to go back once more and remove the selection before that as well
...
* Lock – Locks the selected values in a field, disabling methods elsewhere in the model, such as an App class `Clear` call from modifying the locked fields selections
...
### Properties
The properties of a Generic Object are akin to a definition. They define static and dynamic values for the object. They are written as JSON and saved as part of the model. They always have a `qInfo/qType` property.  Let’s ignore the `qType` property for now and focus on the static and dynamic properties.

Here is pseudo-code for an example Generic Object:

![](102.%20Meet%20the%20Engine/generic-object-properties.png)
...
<animated diagram throughout explanation>
![](102.%20Meet%20the%20Engine/diagram%20idea.png)
![](102.%20Meet%20the%20Engine/static-dyn-prop-diagram.png)
...
![](102.%20Meet%20the%20Engine/static-dyn-prop-interactive.png)
...
Later when we work with Generic Objects and dynamic properties, we’ll use methods on the Generic Object that correspond with those dynamic properties. For example, one time of dynamic property is the `ListObject`, which is useful for representing data from a field in a filter-like format. `ListObjects` are often used to produce filter components, so the Generic Object has specific methods for interacting with this dynamic property, like:
...
* Models are saved to disk in the file format `.qvf`. The API documentation refers to models as “apps” or “documents”.
