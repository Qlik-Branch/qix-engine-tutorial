...
* port – _optional_. The port that the Engine is listening on
...
Let’s take a look at how we would connect to an example server. Qlik Playground has a server with an Engine that we can connect to as an example. This server is hosted on <code>playground.qlik.com</code>, under the default port. It requires SSL. We aren’t going to scope to a specific app. Our resulting WebSocket URL will be:
</br>
<code>wss://playground.qlik.com/app/</code>
</br>
</br>

Connecting to this Engine in JavaScript is easy; we just create a new WebSocket connection to the URL:
</br>
<code>var connection = new WebSocket(“wss://playground.qlik.com/app/”)</code>
</br>
</br>

<div id="chrome-dev-tools" class="graph">![](../../imgs/chrome-dev-tools.png)</div>

We now have a connection with the Engine. We can see this connection and any messages transmitted over it using tools like Google Chrome’s Network panel. How do we read and write the messages though? Enter JSON-RPC.
</br>
</br>
...
1. Open the model using the Global class. We send the Engine the message seen in <code>Open Doc Request</code> in the code editor to the right
</br>
Note that we provide a parameter for the identifier of the model.
The Engine opens the model and returns the handle 1 for it (seen in <code>Open Doc Response</code>)
</br>
</br>

2. Now that we have a model open and a handle for it, we can use that handle to get a handle for our field, as we do in <code>Get Field Request</code>
</br>
The Engine will return us a handle of 2 for the field (<code>Get Field Response</code>)
</br>
</br>

3. Now that we have a handle for our field, we can get the cardinality of it by sending GetCardinal as is done in <code>Get Cardinal Request</code>
</br>
The Engine returns <code>Get Cardinal Response</code>
</br>
</br>
...
1. Establish a WebSocket connection with the Engine (<code>Connect To Engine</code>)
2. Make an API call to get the Engine version (<code>Request Engine Version</code>)
3. Listen for and identify the response with the Engine version (<code>Engine Version Response</code>)
</br>

The full solution can be found in <code>End To End</code>
</br>
</br>
